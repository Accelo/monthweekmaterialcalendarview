...
public class MonthWeekMaterialCalendarWithSwipeView extends FrameLayout implements SlideMonthInterface {
    ...
    public MonthWeekMaterialCalendarWithSwipeView(Context context) {
        this(context, null);
    }

    public MonthWeekMaterialCalendarWithSwipeView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
...
//                            mSlideScrolledlistener.onScrolled(1, percent);
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            int moveY = finalMonthModeHeight - mRefresh.getTop();
    ...
    private void setWeek2MonthMode() {
        ...
        setUpdateBottomAndTopAnimator(mRefresh.getTop(), finalMonthModeHeight, DEFAULT_INTERVAL, new SlideOffsetAnimatorlistener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                int animatedValue = (int) valueAnimator.getAnimatedValue();
                int top = mRefresh.getTop();
                int offset = animatedValue - top;
                mRefresh.offsetTopAndBottom(offset);
            }
        });
...
//        postInvalidate();
    ...
    private void setMonth2WeekMode() {
        setUpdateBottomAndTopAnimator(mRefresh.getTop(), finalWeekModeHeight, DEFAULT_INTERVAL, new SlideOffsetAnimatorlistener() {
            @Override
...
//        });
    ...
    protected void onFinishInflate() {
        ...
        mRefresh = (SwipeRefreshLayout) getChildAt(2);//.findViewById(R.id.recyclerView);
        mRecyclerView = (RecyclerView) mRefresh.findViewById(R.id.recyclerView);
...
//       整除的结果就是选中的日期行数
    ...
    public class SlideStateBuilder {
        ...
        public void commit() {
            MonthWeekMaterialCalendarWithSwipeView.this.commit(new SlideState(this));
        }
    }
